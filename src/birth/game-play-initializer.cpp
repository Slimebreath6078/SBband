...
void init_dungeon_quests(PlayerType *player_ptr)
{
    ...
    floor_ptr->quest_number = i2enum<quest_id>(MIN_RANDOM_QUEST);
    parse_fixed_map(player_ptr, "q_info.txt", 0, 0, 0, 0);
    floor_ptr->quest_number = quest_id::NONE;
    ...
    init_flags = INIT_ASSIGN;
    floor_ptr->quest_number = quest_id::OBERON;
    parse_fixed_map(player_ptr, "q_info.txt", 0, 0, 0, 0);
    quest[enum2i(quest_id::OBERON)].status = QuestStatusType::TAKEN;

    floor_ptr->quest_number = quest_id::SERPENT;
    parse_fixed_map(player_ptr, "q_info.txt", 0, 0, 0, 0);
    quest[enum2i(quest_id::SERPENT)].status = QuestStatusType::TAKEN;
    floor_ptr->quest_number = quest_id::NONE;
}
