...
static bool confirm_leave_level(PlayerType *player_ptr, bool down_stair)
{
    ...
    if (confirm_quest && player_ptr->current_floor_ptr->quest_number != quest_id::NONE && (q_ptr->type == QuestKindType::RANDOM || (q_ptr->flags & QUEST_FLAG_ONCE && q_ptr->status != QuestStatusType::COMPLETED) || (q_ptr->flags & QUEST_FLAG_TOWER && ((q_ptr->status != QuestStatusType::STAGE_COMPLETED) || (down_stair && (quest[enum2i(quest_id::TOWER1)].status != QuestStatusType::COMPLETED)))))) {
...
void do_cmd_go_up(PlayerType *player_ptr)
{
    ...
    if (f_ptr->flags.has(FloorFeatureType::QUEST)) {
        ...
        player_ptr->current_floor_ptr->quest_number = i2enum<quest_id>(g_ptr->special);
    ...
    if (player_ptr->current_floor_ptr->quest_number != quest_id::NONE && quest[enum2i(player_ptr->current_floor_ptr->quest_number)].type == QuestKindType::RANDOM) {
        leave_quest_check(player_ptr);
