...
{
    if (!is_in_dungeon(player_ptr) && player_ptr->current_floor_ptr->quest_number == quest_id::NONE)
        return true;
...
bool vault_aux_dragon(PlayerType *player_ptr, MONRACE_IDX r_idx)
{
    ...
    auto flags = MonsterAbilityType_BREATH_MASK;
...
bool monster_can_entry_arena(PlayerType *player_ptr, MONRACE_IDX r_idx)
{
    ...
    if (!dam && r_ptr->ability_flags.has_none_of(MonsterAbilityType_BOLT_MASK | MonsterAbilityType_BEAM_MASK | MonsterAbilityType_BALL_MASK | MonsterAbilityType_BREATH_MASK))
        return false;
...
{
    return (mon_hook_dungeon(player_ptr, r_idx) && r_info[r_idx].race_kind_flags.has_not(MonraceKindType::UNIQUE) && none_bits(r_info[r_idx].flags7, RF7_UNIQUE2) && r_info[r_idx].resistance_flags.has_not(MonsterResistanceType::RESIST_ALL) && none_bits(r_info[r_idx].flags7, RF7_AQUATIC));
}
