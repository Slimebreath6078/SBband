...
int lore_do_probe(PlayerType *player_ptr, MONRACE_IDX r_idx)
{
    ...
    byte tmp_byte = ((r_ptr->drop_flags.has(MonraceDropType::DROP_4D2) ? 8 : 0) + (r_ptr->drop_flags.has(MonraceDropType::DROP_3D2) ? 6 : 0) + (r_ptr->drop_flags.has(MonraceDropType::DROP_2D2) ? 4 : 0)
        + (r_ptr->drop_flags.has(MonraceDropType::DROP_1D2) ? 2 : 0) + (r_ptr->drop_flags.has(MonraceDropType::DROP_90) ? 1 : 0) + (r_ptr->drop_flags.has(MonraceDropType::DROP_60) ? 1 : 0));

    if (r_ptr->drop_flags.has_not(MonraceDropType::ONLY_GOLD)) {
        if (r_ptr->r_drop_item != tmp_byte)
            n++;
        r_ptr->r_drop_item = tmp_byte;
    }
    if (r_ptr->drop_flags.has_not(MonraceDropType::ONLY_ITEM)) {
    ...
    r_ptr->resistance_flags = r_ptr->resistance_flags;
...
void lore_treasure(PlayerType *player_ptr, MONSTER_IDX m_idx, ITEM_NUMBER num_item, ITEM_NUMBER num_gold)
{
    ...
    if (r_ptr->drop_flags.has(MonraceDropType::DROP_GOOD))
        r_ptr->r_drop_flags.set(MonraceDropType::DROP_GOOD);
    if (r_ptr->drop_flags.has(MonraceDropType::DROP_GREAT))
        r_ptr->r_drop_flags.set(MonraceDropType::DROP_GREAT);
