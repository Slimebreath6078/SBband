...
void set_bolt_types(PlayerType *player_ptr, lore_type *lore_ptr)
{
    ...
    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_LITE)) {
        set_damage(player_ptr, lore_ptr, MonsterAbilityType::BO_LITE, _("スターライトアロー%s", "fire starlight arrow%s"));
        lore_ptr->vp[lore_ptr->vn] = lore_ptr->tmp_msg[lore_ptr->vn];
        lore_ptr->color[lore_ptr->vn++] = TERM_YELLOW;
    }

    if (lore_ptr->ability_flags.has(MonsterAbilityType::BO_DARK)) {
        set_damage(player_ptr, lore_ptr, MonsterAbilityType::BO_DARK, _("暗黒の矢%s", "produce dark bolts%s"));
        lore_ptr->vp[lore_ptr->vn] = lore_ptr->tmp_msg[lore_ptr->vn];
        lore_ptr->color[lore_ptr->vn++] = TERM_L_DARK;
    }
}
...
void set_floor_types(PlayerType *player_ptr, lore_type *lore_ptr)
{
    if (lore_ptr->ability_flags.has(MonsterAbilityType::DARKNESS)) {
        if ((player_ptr->pclass != PlayerClassType::NINJA) || (lore_ptr->r_ptr->race_kind_flags.has(MonraceKindType::UNDEAD) || lore_ptr->r_ptr->resistance_flags.has(MonsterResistanceType::HURT_LITE)) || (lore_ptr->r_ptr->flags7 & RF7_DARK_MASK)) {
