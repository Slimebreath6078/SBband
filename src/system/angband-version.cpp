...
std::string AngbandVersion::build_expression(VersionExpression expression) const
{
    ...
    case VersionExpression::FULL: {
        ...
        if (VERSION_STATUS == VersionStatusType::RELEASE) {
            ...
            return format(_("SBband %d.%d.%d.%d(%s)", "SBband %d.%d.%d.%d(%s)"), H_VER_MAJOR, H_VER_MINOR, H_VER_PATCH, H_VER_EXTRA, mode);
        }

        return format(_("SBband %d.%d.%d%s%d", "SBband %d.%d.%d%s%d"), H_VER_MAJOR, H_VER_MINOR, H_VER_PATCH, expr.data(), H_VER_EXTRA);
    }
    default:
