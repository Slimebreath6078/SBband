...
void process_angar(PlayerType *player_ptr, MONSTER_IDX m_idx, bool see_m)
{
    ...
    if (is_pet(m_ptr) && (((r_ptr->race_kind_flags.has(MonraceKindType::UNIQUE) || (r_ptr->flags7 & RF7_NAZGUL)) && monster_has_hostile_align(player_ptr, nullptr, 10, -10, r_ptr)) || r_ptr->resistance_flags.has(MonsterResistanceType::RESIST_ALL)))
        gets_angry = true;
