...
static bool restrict_monster_to_dungeon(PlayerType *player_ptr, MONRACE_IDX r_idx)
{
    ...
    if (d_ptr->flags.has(DungeonFeatureType::NO_MAGIC)) {
        if (r_idx != MON_CHAMELEON && r_ptr->freq_spell && r_ptr->ability_flags.has_none_of(MonsterAbilityType_NOMAGIC_MASK))
            return false;
    }
...
{
    if ((player_ptr->current_floor_ptr->dun_level > 0) || (player_ptr->current_floor_ptr->quest_number != quest_id::NONE))
...
static errr do_get_mon_num_prep(PlayerType *player_ptr, const monsterrace_hook_type hook1, const monsterrace_hook_type hook2, const bool restrict_to_dungeon)
{
    ...
    for (auto i = 0U; i < alloc_race_table.size(); i++) {
        ...
        if (!player_ptr->phase_out && !chameleon_change_m_idx && summon_specific_type != SUMMON_GUARDIANS) {
            ...
            if (player_ptr->current_floor_ptr->quest_number != quest_id::NONE && r_ptr->resistance_flags.has(MonsterResistanceType::RESIST_ALL))
                continue;
        }
        ...
        if (restrict_to_dungeon) {
            ...
            //   * 1階かそれより深いところにいる
            //   * ランダムクエスト中でない
            const bool in_random_quest = floor_ptr->quest_number != quest_id::NONE && !quest_type::is_fixed(enum2i(floor_ptr->quest_number));
