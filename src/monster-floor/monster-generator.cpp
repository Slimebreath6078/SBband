...
bool place_monster(PlayerType *player_ptr, POSITION y, POSITION x, BIT_FLAGS mode)
{
    ...
    if ((one_in_(5) || (player_ptr->current_floor_ptr->dun_level == 0)) && r_info[r_idx].race_kind_flags.has_not(MonraceKindType::UNIQUE) && angband_strchr("hkoptuyAHLOPTUVY", r_info[r_idx].d_char)) {
        mode |= PM_JURAL;
    }
