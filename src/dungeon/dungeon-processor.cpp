...
void process_dungeon(PlayerType *player_ptr, bool load_game)
{
    ...
    if ((max_dlv[player_ptr->dungeon_idx] < floor_ptr->dun_level) && floor_ptr->quest_number == quest_id::NONE) {
    ...
    if ((quest_num != quest_id::NONE) && r_info[quest[enum2i(quest_num)].r_idx].race_kind_flags.has_not(MonraceKindType::UNIQUE)) {
        r_info[quest[enum2i(quest_num)].r_idx].flags1 &= ~RF1_QUESTOR;
    }
