...
void QuestCompletionChecker::complete()
{
    ...
    auto reward = false;
    if ((this->quest_idx != quest_id::NONE) && (quest[enum2i(this->quest_idx)].status == QuestStatusType::TAKEN)) {
...
void QuestCompletionChecker::set_quest_idx()
{
    ...
    if (this->quest_idx != quest_id::NONE) {
        return;
    }
    ...
    this->quest_idx = i2enum<quest_id>(i);
}
...
std::tuple<bool, bool> QuestCompletionChecker::complete_random()
{
    ...
    if (none_bits(this->q_ptr->flags, QUEST_FLAG_PRESET)) {
        create_stairs = true;
        this->player_ptr->current_floor_ptr->quest_number = quest_id::NONE;
    }

    if ((this->quest_idx == quest_id::OBERON) || (this->quest_idx == quest_id::SERPENT)) {
        this->q_ptr->status = QuestStatusType::FINISHED;
    }
...
void QuestCompletionChecker::complete_tower()
{
    ...
    auto is_tower_completed = quest[enum2i(quest_id::TOWER1)].status == QuestStatusType::STAGE_COMPLETED;
    is_tower_completed &= quest[enum2i(quest_id::TOWER2)].status == QuestStatusType::STAGE_COMPLETED;
    is_tower_completed &= quest[enum2i(quest_id::TOWER3)].status == QuestStatusType::STAGE_COMPLETED;
    if (is_tower_completed) {
        complete_quest(this->player_ptr, quest_id::TOWER1);
    }
}
