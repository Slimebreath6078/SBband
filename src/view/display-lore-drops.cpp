...
{
    if (lore_ptr->flags1 & MonraceDropType::DROP_GREAT) {
        lore_ptr->drop_quality = _("特別な", " exceptional");
    } else if (lore_ptr->flags1 & MonraceDropType::DROP_GOOD) {
...
void display_monster_drop_golds(lore_type *lore_ptr)
{
    ...
    is_item_only |= any_bits(lore_ptr->flags1, MonraceDropType::DROP_GOOD);
    is_item_only |= any_bits(lore_ptr->flags1, MonraceDropType::DROP_GREAT);
    if (is_item_only)
        return;
