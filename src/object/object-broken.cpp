...
ObjectBreaker::ObjectBreaker(tr_type ignore_flg, tr_type resist_flg, tr_type immute_flg)
    : ignore_flg(ignore_flg)
    , resist_flg(resist_flg)
    , immute_flg(immute_flg)
{
}
...
bool ObjectBreaker::can_destroy(object_type *o_ptr) const
{
    ...
    if (flgs.has(this->ignore_flg) || flgs.has(this->resist_flg) || flgs.has(this->immute_flg))
        return false;
    return true;
}
