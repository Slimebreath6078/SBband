 ...
 */
static void get_questinfo(PlayerType *player_ptr, quest_id questnum, bool do_init)
{
    for (int i = 0; i < 10; i++) {
        quest_text[i][0] = '\0';
    }
 ...
 * @param do_init クエストの開始処理か(true)、結果処理か(FALSE)
 */
void print_questinfo(PlayerType *player_ptr, quest_id questnum, bool do_init)
{
...
{
    clear_bldg(4, 18);
    quest_id q_index = i2enum<quest_id>(player_ptr->current_floor_ptr->grid_array[player_ptr->y][player_ptr->x].special);

    if (q_index == quest_id::NONE) {
