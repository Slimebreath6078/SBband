...
MonsterSpellResult spell_RF6_TELE_LEVEL(player_type *target_ptr, MONSTER_IDX m_idx, MONSTER_IDX t_idx, int TARGET_TYPE)
{
    ...
    if (TARGET_TYPE == MONSTER_TO_PLAYER) {
        ...
        spell_badstatus_message_to_player(target_ptr, m_idx, _("%^sが何か奇妙な言葉をつぶやいた。", "%^s mumbles strangely."),
    ...
    spell_badstatus_message_to_mons(target_ptr, m_idx, t_idx, _("%^sが%sの足を指さした。", "%^s gestures at %s's feet."),
