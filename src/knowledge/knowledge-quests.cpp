...
static void do_cmd_knowledge_quests_current(PlayerType *player_ptr, FILE *fff)
{
    ...
    for (int16_t i = 1; i < max_q_idx; i++) {
        ...
        quest_text_line = 0;
        player_ptr->current_floor_ptr->quest_number = i2enum<quest_id>(i);
...
static bool do_cmd_knowledge_quests_aux(PlayerType *player_ptr, FILE *fff, quest_id q_idx)
{
    char tmp_str[120];
    ...
    auto is_fixed_quest = quest_type::is_fixed(enum2i(q_idx));
    if (is_fixed_quest) {
        quest_id old_quest = floor_ptr->quest_number;
...
void do_cmd_knowledge_quests_completed(PlayerType *player_ptr, FILE *fff, int16_t quest_num[])
{
    ...
    int16_t total = 0;
    for (int16_t i = 1; i < max_q_idx; i++) {
        quest_id q_idx = i2enum<quest_id>(quest_num[i]);
        quest_type *const q_ptr = &quest[enum2i(q_idx)];
...
void do_cmd_knowledge_quests(PlayerType *player_ptr)
{
    ...
    std::iota(quest_num.begin(), quest_num.end(), enum2i(quest_id::NONE));
