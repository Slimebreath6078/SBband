 ...
 * @return クエストID
 */
static quest_id write_floor(PlayerType *player_ptr, concptr *note_level, char *note_level_buf)
{
    floor_type *floor_ptr = player_ptr->current_floor_ptr;
    quest_id q_idx = quest_number(player_ptr, floor_ptr->dun_level);
    if (!write_level)
        return q_idx;
    ...
    else if (q_idx != quest_id::NONE && quest_type::is_fixed(enum2i(q_idx)) && !((q_idx == quest_id::OBERON) || (q_idx == quest_id::SERPENT)))
...
errr exe_write_diary(PlayerType *player_ptr, int type, int num, concptr note)
{
    ...
    if (type == DIARY_FIX_QUEST_C ||
        ...
        type == DIARY_TO_QUEST) {
        quest_id old_quest = player_ptr->current_floor_ptr->quest_number;
        player_ptr->current_floor_ptr->quest_number = (quest[num].type == QuestKindType::RANDOM) ? quest_id::NONE : i2enum<quest_id>(num);
    ...
    quest_id q_idx = write_floor(player_ptr, &note_level, note_level_buf);
    ...
    case DIARY_STAIR: {
        concptr to = q_idx != quest_id::NONE && (quest_type::is_fixed(enum2i(q_idx)) && !((q_idx == quest_id::OBERON) || (q_idx == quest_id::SERPENT)))
                         ? _("地上", "the surface")
