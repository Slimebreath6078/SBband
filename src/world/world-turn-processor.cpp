...
{
    auto *floor_ptr = this->player_ptr->current_floor_ptr;
    if (!floor_ptr->dun_level && floor_ptr->quest_number == quest_id::NONE && !this->player_ptr->phase_out && !floor_ptr->inside_arena) {
    ...
    is_in_dungeon |= lite_town && (floor_ptr->quest_number == quest_id::NONE) && !this->player_ptr->phase_out && !floor_ptr->inside_arena;
...
void WorldTurnProcessor::decide_alloc_monster()
{
    ...
    should_alloc &= floor_ptr->quest_number == quest_id::NONE;
