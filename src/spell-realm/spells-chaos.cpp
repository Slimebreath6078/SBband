...
void call_the_void(PlayerType *player_ptr)
{
    ...
    bool is_special_fllor = floor_ptr->quest_number != quest_id::NONE && quest_type::is_fixed(enum2i(floor_ptr->quest_number));
...
{
    auto *floor_ptr = player_ptr->current_floor_ptr;
    bool is_special_floor = floor_ptr->quest_number != quest_id::NONE && quest_type::is_fixed(enum2i(floor_ptr->quest_number));
