...
bool genocide_aux(PlayerType *player_ptr, MONSTER_IDX m_idx, int power, bool player_cast, int dam_side, concptr spell_name)
{
    ...
    if (r_ptr->race_kind_flags.has(MonraceKindType::UNIQUE) || (r_ptr->flags1 & RF1_QUESTOR))
        ...
        resist = true;
    else if ((player_ptr->current_floor_ptr->quest_number != quest_id::NONE && random_quest_number(player_ptr, player_ptr->current_floor_ptr->dun_level) == quest_id::NONE) || player_ptr->current_floor_ptr->inside_arena || player_ptr->phase_out)
        resist = true;
...
bool symbol_genocide(PlayerType *player_ptr, int power, bool player_cast)
{
    ...
    bool is_special_floor = floor_ptr->quest_number != quest_id::NONE && random_quest_number(player_ptr, floor_ptr->dun_level) == quest_id::NONE;
