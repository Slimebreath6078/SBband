...
void stair_creation(PlayerType *player_ptr)
{
    ...
    bool down = true;
    floor_type *floor_ptr = player_ptr->current_floor_ptr;
    if (quest_number(player_ptr, floor_ptr->dun_level) != quest_id::NONE || (floor_ptr->dun_level >= d_info[player_ptr->dungeon_idx].maxdepth))
        down = false;

    if (!floor_ptr->dun_level || (!up && !down) || (floor_ptr->quest_number != quest_id::NONE && quest_type::is_fixed(enum2i(floor_ptr->quest_number))) || floor_ptr->inside_arena || player_ptr->phase_out) {
