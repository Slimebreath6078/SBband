...
melee_spell_type *initialize_melee_spell_type(PlayerType *player_ptr, melee_spell_type *ms_ptr, MONSTER_IDX m_idx)
{
    ...
    ms_ptr->in_no_magic_dungeon = d_info[player_ptr->dungeon_idx].flags.has(DungeonFeatureType::NO_MAGIC) && floor_ptr->dun_level && (floor_ptr->quest_number == quest_id::NONE || quest_type::is_fixed(enum2i(floor_ptr->quest_number)));
    return ms_ptr;
}
