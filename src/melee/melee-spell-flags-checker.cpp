...
static void decide_indirection_melee_spell(PlayerType *player_ptr, melee_spell_type *ms_ptr)
{
    ...
    ms_ptr->ability_flags &= MonsterAbilityType_INDIRECT_MASK;
}
...
static void check_stupid(melee_spell_type *ms_ptr)
{
    ...
    ms_ptr->ability_flags &= MonsterAbilityType_NOMAGIC_MASK;
}
...
static void check_arena(PlayerType *player_ptr, melee_spell_type *ms_ptr)
{
    ...
    ms_ptr->ability_flags.reset(MonsterAbilityType_SUMMON_MASK).reset(MonsterAbilityType::TELE_LEVEL);
...
{
    auto ball_mask_except_rocket = MonsterAbilityType_BALL_MASK;
    ...
    ms_ptr->ability_flags.reset(MonsterAbilityType_BIG_BALL_MASK);
}
...
{
    if (ms_ptr->ability_flags.has_none_of(MonsterAbilityType_BEAM_MASK) || direct_beam(player_ptr, ms_ptr->m_ptr->fy, ms_ptr->m_ptr->fx, ms_ptr->t_ptr->fy, ms_ptr->t_ptr->fx, ms_ptr->m_ptr))
        return;

    ms_ptr->ability_flags.reset(MonsterAbilityType_BEAM_MASK);
}

static void check_melee_spell_breath(PlayerType *player_ptr, melee_spell_type *ms_ptr)
{
    if (ms_ptr->ability_flags.has_none_of(MonsterAbilityType_BREATH_MASK))
        return;
    ...
    if (!breath_direct(player_ptr, ms_ptr->m_ptr->fy, ms_ptr->m_ptr->fx, ms_ptr->t_ptr->fy, ms_ptr->t_ptr->fx, rad, AttributeType::NONE, true)) {
        ms_ptr->ability_flags.reset(MonsterAbilityType_BREATH_MASK);
        return;
    }
...
static void check_riding(PlayerType *player_ptr, melee_spell_type *ms_ptr)
{
    ...
    ms_ptr->ability_flags.reset(MonsterAbilityType_RIDING_MASK);
}
...
static void check_pet(PlayerType *player_ptr, melee_spell_type *ms_ptr)
{
    ...
    if (!(player_ptr->pet_extra_flags & PF_ATTACK_SPELL)) {
        ms_ptr->ability_flags.reset(MonsterAbilityType_ATTACK_MASK);
    }

    if (!(player_ptr->pet_extra_flags & PF_SUMMON_SPELL)) {
        ms_ptr->ability_flags.reset(MonsterAbilityType_SUMMON_MASK);
    }
...
static void check_non_stupid(PlayerType *player_ptr, melee_spell_type *ms_ptr)
{
    ...
    if (ms_ptr->ability_flags.has_any_of(MonsterAbilityType_BOLT_MASK) && !clean_shot(player_ptr, ms_ptr->m_ptr->fy, ms_ptr->m_ptr->fx, ms_ptr->t_ptr->fy, ms_ptr->t_ptr->fx, ms_ptr->pet)) {
        ms_ptr->ability_flags.reset(MonsterAbilityType_BOLT_MASK);
    }

    if (ms_ptr->ability_flags.has_any_of(MonsterAbilityType_SUMMON_MASK) && !(summon_possible(player_ptr, ms_ptr->t_ptr->fy, ms_ptr->t_ptr->fx))) {
        ms_ptr->ability_flags.reset(MonsterAbilityType_SUMMON_MASK);
    }
...
static void check_smart(PlayerType *player_ptr, melee_spell_type *ms_ptr)
{
    ...
    if ((ms_ptr->m_ptr->hp < ms_ptr->m_ptr->maxhp / 10) && (randint0(100) < 50)) {
        ms_ptr->ability_flags &= MonsterAbilityType_INT_MASK;
    }
