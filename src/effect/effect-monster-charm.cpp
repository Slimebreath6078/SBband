...
process_result effect_monster_control_undead(PlayerType *player_ptr, effect_monster_type *em_ptr)
{
    ...
    if (common_saving_throw_control(player_ptr, em_ptr->dam, em_ptr->m_ptr) || em_ptr->r_ptr->race_kind_flags.has_not(MonraceKindType::UNDEAD)) {
        em_ptr->note = _("には効果がなかった。", " is unaffected.");
        em_ptr->obvious = false;
        if (one_in_(4))
            em_ptr->m_ptr->mflag2.set(MonsterConstantFlagType::NOPET);
...
process_result effect_monster_control_demon(PlayerType *player_ptr, effect_monster_type *em_ptr)
{
    ...
    if (common_saving_throw_control(player_ptr, em_ptr->dam, em_ptr->m_ptr) || em_ptr->r_ptr->race_kind_flags.has_not(MonraceKindType::DEMON)) {
        em_ptr->note = _("には効果がなかった。", " is unaffected.");
        em_ptr->obvious = false;
        if (one_in_(4))
            em_ptr->m_ptr->mflag2.set(MonsterConstantFlagType::NOPET);
...
process_result effect_monster_control_animal(PlayerType *player_ptr, effect_monster_type *em_ptr)
{
    ...
    if (common_saving_throw_control(player_ptr, em_ptr->dam, em_ptr->m_ptr) || em_ptr->r_ptr->race_kind_flags.has_not(MonraceKindType::ANIMAL)) {
        em_ptr->note = _("には効果がなかった。", " is unaffected.");
        em_ptr->obvious = false;
        if (one_in_(4))
            em_ptr->m_ptr->mflag2.set(MonsterConstantFlagType::NOPET);
...
process_result effect_monster_capture(PlayerType *player_ptr, effect_monster_type *em_ptr)
{
    ...
    if ((floor_ptr->quest_number && (quest[floor_ptr->quest_number].type == QuestKindType::KILL_ALL) && !is_pet(em_ptr->m_ptr)) || em_ptr->r_ptr->race_kind_flags.has(MonraceKindType::UNIQUE) || em_ptr->r_ptr->flags1 & RF1_QUESTOR || any_bits(em_ptr->r_ptr->flags7, RF7_NAZGUL | RF7_UNIQUE2) || em_ptr->m_ptr->parent_m_idx) {
