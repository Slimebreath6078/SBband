...
void switch_effects_player(PlayerType *player_ptr, EffectPlayerType *ep_ptr)
{
    ...
    case AttributeType::COLD:
        effect_player_elements(player_ptr, ep_ptr, _("冷気で攻撃された！", "You are hit by cold!"), cold_dam(player_ptr, ep_ptr->dam, ep_ptr->killer, false));
        return;
    case AttributeType::ELEC:
        effect_player_elements(player_ptr, ep_ptr, _("電撃で攻撃された！", "You are hit by lightning!"), elec_dam(player_ptr, ep_ptr->dam, ep_ptr->killer, false));
        return;
